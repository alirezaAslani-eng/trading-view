...
import {
  getTradePrecent,
  useSettlementMode,
} from "@/v2-architecture/src/features/trading";
...
function TradePanel() {
  ...
  //  * ------- Submit handler ---------
  const { toggleMode, settlementMode } = useSettlementMode()!;
  ...
  const onSubmit = async (fields: TradeFormSchemaOutputType) => {
    ...
    if (!hasAccess) return;
    return safeAsync(() =>
      placeOrderApi.mutateAsync({ ...fields, settlementMode, isDemo }),
    );
  };
      ...
      <Box component={"form"} onSubmit={form.handleSubmit(onSubmit)}>
        <Stack sx={{ width: "100%" }}>
          ...
          <Box sx={{ mt: "calc(32px - 14px)" }}>
            ...
            <FormControlLabel
              sx={{ my: 4 }}
              control={<Switch checked={settlementMode} onClick={toggleMode} />}
              label={
                <Typography
                  component="span"
                  variant="caption"
